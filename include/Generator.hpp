...
template< typename T > void generatePulsar(const unsigned int period, const unsigned int width, const float DM, const AstroData::Observation & observation, const unsigned int padding, std::vector< std::vector< T > * > & data, const bool random) {
  ...
  for ( unsigned int second = 0; second < observation.getNrSeconds(); second++ ) {
    data[second] = new std::vector< T >(observation.getNrChannels() * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T)));
    if ( random ) {
      for ( unsigned int channel = 0; channel < observation.getNrChannels(); channel++ ) {
        for ( unsigned int sample = 0; sample < observation.getNrSamplesPerSecond(); sample++ ) {
          data[second]->at((channel * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T))) + sample) = static_cast< T >(std::rand() % 25);
        }
      }
    } else {
  ...
  for ( unsigned int channel = 0; channel < observation.getNrChannels(); channel++ ) {
    ...
    for ( unsigned int sample = shift; sample < observation.getNrSeconds() * observation.getNrSamplesPerSecond(); sample += period ) {
      for ( unsigned int i = 0; i < width; i++ ) {
        ...
        if ( random ) {
          data[second]->at((channel * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T))) + internalSample) = static_cast< T >(std::rand() % 128);
        } else {
          data[second]->at((channel * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T))) + internalSample) = static_cast< T >(42);
        }
      }
    }
  }
...
template< typename T > void generateSinglePulse(const unsigned int width, const float DM, const AstroData::Observation & observation, const unsigned int padding, std::vector< std::vector< T > * > & data, const uint8_t inputBits, const bool random) {
  ...
  for ( unsigned int second = 0; second < observation.getNrSeconds(); second++ ) {
    if ( inputBits >= 8 ) {
      ...
      data[second] = new std::vector< T >(observation.getNrChannels() * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T)));
    }
    if ( random ) {
      for ( unsigned int channel = 0; channel < observation.getNrChannels(); channel++ ) {
        for ( unsigned int sample = 0; sample < observation.getNrSamplesPerSecond(); sample++ ) {
          if ( inputBits >= 8 ) {
            ...
            unsigned char buffer = data[second]->at((channel * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T))) + byte);

            for ( uint8_t bit = 0; bit < inputBits; bit++ ) {
              isa::utils::setBit(buffer, isa::utils::getBit(value, bit), firstBit + bit);
            }
            data[second]->at((channel * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T))) + byte) = buffer;
          }
        }
      }
    } else {
  ...
  for ( unsigned int channel = 0; channel < observation.getNrChannels(); channel++ ) {
    ...
    for ( unsigned int i = 0; i < width; i++ ) {
      ...
      if ( random ) {
        if ( inputBits >= 8 ) {
          data[second + ((sample + i + shift) / observation.getNrSamplesPerSecond())]->at((channel * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T))) + (sample + i + shift)) = static_cast< T >(std::rand() % 256);
        } else {
          ...
          unsigned char buffer = data[second + ((sample + i + shift) / observation.getNrSamplesPerSecond())]->at((channel * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T))) + byte);

          for ( uint8_t bit = 0; bit < inputBits; bit++ ) {
            isa::utils::setBit(buffer, isa::utils::getBit(value, bit), firstBit + bit);
          }
          data[second + ((sample + i + shift) / observation.getNrSamplesPerSecond())]->at((channel * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T))) + byte) = buffer;
        }
      } else {
        if ( inputBits >= 8 ) {
          data[second + ((sample + i + shift) / observation.getNrSamplesPerSecond())]->at((channel * observation.getNrSamplesPerPaddedSecond(padding / sizeof(T))) + ((sample + i + shift) % observation.getNrSamplesPerSecond())) = static_cast< T >(42);
        } else {
          unsigned int byte = ((sample + i + shift) % observation.getNrSamplesPerSecond()) / (8 / inputBits);
          uint8_t firstBit = (((sample + i + shift) % observation.getNrSamplesPerSecond()) % (8 / inputBits)) * inputBits;
          unsigned char buffer = 0;

          buffer = data[second + ((sample + i + shift) / observation.getNrSamplesPerSecond())]->at((channel * isa::utils::pad(observation.getNrSamplesPerSecond() / (8 / inputBits), padding / sizeof(T))) + byte);
