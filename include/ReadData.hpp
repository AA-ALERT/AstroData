...
template< typename T > void readSIGPROC(const Observation & observation, const unsigned int padding, const uint8_t inputBits, const unsigned int bytesToSkip, const std::string & inputFilename, std::vector< std::vector< T > * > & data, const unsigned int firstSecond = 0) {
    ...
    if ( inputBits >= 8 ) {
      ...
      for ( unsigned int sample = 0; sample < observation.getNrSamplesPerBatchd(); sample++ ) {
      ...
      uint64_t bytesToRead = static_cast< uint64_t >(observation.getNrSamplesPerBatchd() * (observation.getNrChannels() / (8.0 / inputBits)));

      data.at(second) = new std::vector< T >(observation.getNrChannels() * isa::utils::pad(observation.getNrSamplesPerBatchd() / (8 / inputBits), padding / sizeof(T)));
        ...
        for ( unsigned int item = 0; item < 8 / inputBits; item++ ) {
          ...
          if ( item > channel ) {
            ...
            while ( item < (8 / inputBits) ) {
              ...
              sampleBuffer = data.at(second)->at((static_cast< uint64_t >(channel - channelOffset) * isa::utils::pad(observation.getNrSamplesPerBatchd() / (8 / inputBits), padding / sizeof(T))) + sampleByte);
              ...
              data.at(second)->at((static_cast< uint64_t >(channel - channelOffset) * isa::utils::pad(observation.getNrSamplesPerBatchd() / (8 / inputBits), padding / sizeof(T))) + sampleByte) = sampleBuffer;
              item++;
              channelOffset++;
            }
          ...
          sampleBuffer = data.at(second)->at((static_cast< uint64_t >(channel - item) * isa::utils::pad(observation.getNrSamplesPerBatchd() / (8 / inputBits), padding / sizeof(T))) + sampleByte);
          for ( uint8_t bit = 0; bit < inputBits; bit++ ) {
            isa::utils::setBit(sampleBuffer, isa::utils::getBit(*buffer, channelFirstBit + bit), sampleFirstBit + bit);
          }
          data.at(second)->at((static_cast< uint64_t >(channel - item) * isa::utils::pad(observation.getNrSamplesPerBatchd() / (8 / inputBits), padding / sizeof(T))) + sampleByte) = sampleBuffer;
        }
      }
    }
	}
...
template< typename T > void readLOFAR(std::string headerFilename, std::string rawFilename, Observation & observation, const unsigned int padding, std::vector< std::vector< T > * > & data, unsigned int nrSeconds, unsigned int firstSecond) {
  ...
  channelBandwidth = valueDouble / 1000000;
	...
	observation.setNrSamplesPerBatchd(static_cast< unsigned int >(totalSamples / totalIntegrationTime));
	if ( nrSeconds == 0 ) {
  ...
  observation.setFrequencyRange(1, nrSubbands * nrChannels, minFreq, channelBandwidth);
	...
	if ( firstSecond > 0 ) {
		rawFile.seekg(firstSecond * observation.getNrSamplesPerBatchd() * nrSubbands * nrChannels, std::ios::beg);
	}
...
template< typename T > void readPSRDadaHeader(Observation & observation, dada_hdu_t & ringBuffer) throw(RingBufferError) {
  ...
  observation.setNrSamplesPerBatchd(uintValue);
